            ...
            try:
                if isinstance(source_module, nn.Module) and isinstance(target_module, nn.Module):
                    ...
                    if isinstance(source_module, nn.Conv2d) and isinstance(target_module, nn.Conv2d):
                        ...
                        elif target_module.in_channels > source_module.in_channels:
                            # Expand input channels: avg existing, append as extra channel(s)
                            n_extra = target_module.in_channels - source_module.in_channels
                            avg_ch = source_module.weight.mean(dim=1, keepdim=True)
                            extra = avg_ch.repeat(1, n_extra, 1, 1)
                            new_weight = torch.cat([source_module.weight, extra], dim=1)
                            target_module.weight = nn.Parameter(new_weight)
                            if source_module.bias is not None and target_module.bias is not None:
                                target_module.bias = nn.Parameter(source_module.bias.clone())
                            print(f"Expand input channels ({source_module.in_channels}->{target_module.in_channels}): {source_name} -> {target_name}")
